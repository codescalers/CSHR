...
class VacationBalanceTest(APITestCase):
    ...
    def test_write_to_balance_file(self):
        ...
        """
        content = {"annual_leaves": 15, "sick_leaves": 100, "compensation": 0, "unpaid": 0, "emergencies": 6, "leave_execuses": 3, "public_holidays": ["2022-09-26"], "year": 2022}
        v = VacationBalanceHelper()
        v.bulk_write(content)
        self.assertEqual(v.file_content["annual_leaves"], 15)
        self.assertEqual(len(v.file_content["public_holidays"]), 1)

    def test_user_balance(self):
        u = User.objects.get(pk=1)
        v = VacationBalanceHelper()
        self.assertEqual(u.vacationbalance.public_holidays, 1)
    ...
    def test_public_holidays_count(self):
        ...
        """
        This is 6 bcos the functions main goal is to take the list of 
        public holidays from the file as input. when it does so the user
        has no balance actually. so here it doubles the balance
        """
    ...
    #     balance = {"annual_leaves": 15, "sick_leaves": 100, "compensation": 0, "unpaid": 0, "emergencies": 4, "leave_execuses": 5, "public_holidays": ["2022-10-12 12:50:54.535463", "2022-10-14 12:50:54.535463", "2022-10-13 12:50:54.535463"], "year": 2022}
    #     v = VacationBalanceHelper()
    ...
    #     self.assertEqual(u.vacationbalance.emergencies, 2.7)
    #     self.assertEqual(u.vacationbalance.leave_execuses, 3.3)
    ...
    #     )
    #     self.assertEqual(res, "You only have 15.0 days left of type annual_leaves")
    ...
    #     )
    #     self.assertEqual(res, "You only have 30.0 days left of type annual_leaves")
    ...
    # def test_apply_for_vacation_when_request_exceeds_old_balance(self):
    #     v = VacationBalanceHelper()
    #     u = User.objects.get(pk=1)
    #     v.old_balance_format(u)
    ...
    #         u,
    #         datetime.datetime.now(),
    #         datetime.datetime.now() + timedelta(30),
    #         "annual_leaves",
    #     )
    #     self.assertEqual(res, 0)
    #     self.assertEqual(u.vacationbalance.annual_leaves, 0)
