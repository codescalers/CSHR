...
export default async function isAuthenticated() {
	...
	if (refreshtoken) {
		...
		if(Date.now() < exprefresh * 1000){
			const myloggingData: refreshData =  await refreshDataService.refresh(refreshtoken);
			console.log(myloggingData);
			return myAuth={"token": myloggingData.access, "refreshtoken":myloggingData.refresh};
