    ...
    <div slot="body" class="bg-confetti-animated">
        {#if clickedItemOnModal.users}
            <div class="container d-flex flex-column gap-5 px-5 my-5">
                <h3 class="mx-auto text-muted">{clickedItemOnModal.users.length > 1
                    ? 'Wish them a Happy Birthday !'
                    : `Wish ${clickedItemOnModal.users[0].gender === 'Male' ? 'him' : 'her'} a BirthDay ! `}</h3>
                <div class="row m-0 justify-content-center">
                    {#each clickedItemOnModal.users as user}
                        <!-- svelte-ignore a11y-click-events-have-key-events -->
                        <div class={`col-2 p-0 justify-content-center d-flex photo-active ${user.id === currentUserID ? 'active-user' : ''}`} on:click={
                            () => {
                                activateUser(user)
                            }
                        }>
