          ...
          <v-card class="pa-4 hover-card border bg-graytitle">
            <v-card-title>
              <v-row>
                <v-col cols="2">
                  <v-img
                    v-if="isCustomIcon(office)"
                    alt="Office Country"
                    :src="getCountryFlagSrc(office)"
                  />
                ...
                <v-col>
                  <p style="margin-left: -15px">
                    {{
                      office.name.length > 30
                        ? capitalize(office.name).slice(0, 30) + '...'
...
export default {
  ...
  setup() {
    ...
    const isCustomIcon = (office: Api.LocationType): boolean =>
      getOfficeCountry(office) !== office.country
