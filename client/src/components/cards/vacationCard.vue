    ...
    <v-container class="pa-6">
      ...
      <v-form ref="form" @submit.prevent="updateVacation()">
        <v-row class="d-flex justify-center my-2">
          ...
          >
          <v-btn
            v-if="isAdminOfTheSameOffice"
            color="error"
            class="mx-1 my-2"
            @click="updateRequestStatus('canceled')"
          >
            Delete this request
          </v-btn>
        </v-row>
        ...
        <v-card elevation="0" class="pa-4">
          <v-row class="py-2">
            <v-col cols="6" class="d-flex">
              ...
              <p>
                Applying User :
                <span color="warning" class="mx-2">{{ vacation.applying_user.full_name }}</span>
              </p>
            </v-col>
      ...
      <v-row
        ...
        v-if="couldApprove && vacation.status === pendingStatus"
      >
      ...
      <v-row
        class="d-flex justify-end mt-3"
        v-if="couldApprove && vacation.status === requestedToCancelStatus"
      >
        <v-btn color="primary" class="ma-1" @click="updateRequestStatus('cancel_approved')"
          >Approve the cancellation request</v-btn
        >
        <v-btn color="error" class="ma-1" @click="updateRequestStatus('cancel_rejected')"
          >Reject the cancellation request</v-btn
        >
      </v-row>
    </v-container>
  </v-card>
...
export default {
  ...
  setup(props, ctx) {
    ...
    const isAdminOfTheSameOffice = computed(() => {
      if (user.value) {
        if (
    ...
    watch(
      ...
      ([newLeaveReason, newStartDate, newEndDate], [oldLeaveReason, oldStartDate, oldEndDate]) => {
        if (
          newLeaveReason.name !== oldLeaveReason.name ||
