<script lang="ts">
  ...
  import type { AdminViewInterface, SupervisorViewInterface, UserInterface } from '../../types';
  export let item: birthDateItemType;

  let itemIndex: number = 0;
  let clickedUser: userType | AdminViewInterface | SupervisorViewInterface | UserInterface;
...
>
  ...
  <div slot="body" class="bg-confetti-animated">
      <h3 class="mx-auto text-muted text-center">{header}</h3>
      <div class="d-flex justify-content-center">
        <div class="card b-none d-flex justify-content-center align-items-center">
          <ProfileImage user={clickedUser} />
          {#if itemIndex !== null && clickedUser !== undefined && clickedUser !== null}
            <div class="card-body text-center">
              <span class="text-muted">{clickedUser.full_name} -</span>
              <span class="text-muted">{clickedUser.team}</span>
            </div>
          {/if}
        </div>
      </div>
